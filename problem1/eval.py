...
def evaluate_subset(
    ...
    # Create model
    model = create_model(model_name, num_classes=100).to(device)

    # Load checkpoint
    subset_checkpoint_dir = checkpoint_dir / f"subset_{subset_size:.4f}"
    checkpoint_manager = CheckpointManager(
        checkpoint_dir=subset_checkpoint_dir, model_name=model_name
    )

    try:
        # Use the manager to load the best checkpoint
...
def find_subset_sizes_from_dirs(checkpoint_dir):
    """Scan directory and parse subset sizes from subdirectory names."""
    ...
    print(f"Subset sizes found: {sorted(subset_sizes)}")
...
def main():
    ...
    subset_sizes = find_subset_sizes_from_dirs(checkpoint_dir)
    ...
    if all_results:
        ...
        # Print summary
        print("\nResults Summary:")
        print(f"{'Subset Size':<15} {'Top-1 Error':<15} {'Top-5 Error':<15}")
        print("-" * 45)
        for subset_size in sorted(all_results.keys()):
            metrics = all_results[subset_size]
            top1_err = metrics.get("top-1-error", 0)
            top5_err = metrics.get("top-5-error", 0)
            print(f"{subset_size:<15.4f} {top1_err:<15.2f} {top5_err:<15.2f}")
    else:
